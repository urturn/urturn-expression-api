(function(){
  ...
  buster.testCase('Collection', {
    ...
    'fit HTML5 storage interface': {
      ...
      '#key': {
        'return null when index is to high': function() {
          expect(collection.key(10)).toBeNull() ;
        },
        'return the key name': function() {
      ...
      '#setItem': {
        ...
        'do nothing if item is undefined and did not exists': function(){
          collection = new UT.Collection({document_id: document_id, data: emptyData, delegate: dataDelegate, currentUserId: 'abc'});
        ...
        'do nothing if item is null': function(){
          collection = new UT.Collection({document_id: document_id, data: emptyData, delegate: dataDelegate, currentUserId: 'abc'});
          collection.setItem('a', 2);
          expect(collection.length).toBe(1);
    ...
    'sanitization': {
      ...
      'sanitize anonymous object with a marshall function': function(){
    ...
    '#save': {
      'let you save item after setItem': function(){
      ...
      'let you save modified item after []=': function(){
        expect(dataDelegate.operations.length).toBe(0);

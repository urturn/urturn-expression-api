...
(function(UT, undefined){
  ...
  describe('UT.Image', function() {
    ...
    describe('SVG templates', function() {
      ...
      it('not marshalled in standard image after its svg rendering', function(done) {
    ...
    describe('image from URL', function() {
      ...
      it('can retrieve it as a SVG', function(done) {
        this.image.svg(function(svg){
          expectedSVG.call(this, svg, {
            width: 400,
            height: 404,
            url: imageURL
          });
    ...
    describe('image from svg data URL', function() {
      beforeEach(function() {
        this.image = new UT.Image(imageDataURISVG);
      });
      ...
      it('can retrieve it as a SVG', function(done) {
        this.image.svg(function(svg){
          ...
          div.appendChild(svg);
          var actualSVG = div.innerHTML;
          var expectedSVG = imageSVG;
          expect(actualSVG).to.equal(expectedSVG);
          done();
        });
      });
      ...
      it('can retrieve it as an image node', function(done) {
        this.image.node(function(img) {
          expect(img.complete).to.be.ok();
          expect(img.width).to.be(500);
          expect(img.height).to.be(500);
          done();
        });
      });
    });
